...
function setupAuthListener() {
  ...
  const { data } = supabaseClient.auth.onAuthStateChange(async (event, session) => {
    ...
    // Only trigger full sync reload if the user actually transitioned (e.g. login/logout)
    // to prevent redundant race-condition loads on page load or token refreshes!
    if ((oldUser === null && currentUser !== null) || (oldUser !== null && currentUser === null) || (oldUser && currentUser && oldUser.id !== currentUser.id)) {
  ...
  try {
    realtimeChannel = supabaseClient
      ...
      .on('postgres_changes', {
        ...
        // Prevent feedback loop / redundant re-renders for events generated by this tab
        if (payload.new) {
          const localLead = leads.find(l => String(l.id).trim() === String(payload.new.id).trim());
          if (localLead) {
            const isIdentical = 
              localLead.nome === payload.new.nome &&
              localLead.whatsapp === (payload.new.whatsapp || '') &&
              localLead.status === payload.new.status &&
              localLead.origem === (payload.new.origem || '') &&
              localLead.tipo === (payload.new.tipo || '') &&
              localLead.regiao === (payload.new.regiao || '') &&
              localLead.valor === (payload.new.valor || '') &&
              localLead.proximaAcao === (payload.new.proxima_acao || '') &&
              localLead.dataRetorno === (payload.new.data_retorno || '') &&
              localLead.observacoes === (payload.new.observacoes || '') &&
              (localLead.historico || []).length === (payload.new.historico || []).length;
            
            if (isIdentical) {
              return; // Local lead is already up-to-date, ignore event
            }
          }
        }
        if (payload.eventType === 'DELETE' && payload.old) {
          const existsLocal = leads.some(l => String(l.id).trim() === String(payload.old.id).trim());
          if (!existsLocal) {
            return; // Lead already deleted locally, ignore event
          }
        }
        await loadLeads();
        renderAll();
...
async function loadLeads() {
  if (supabaseClient && currentUser) {
    try {
      ...
      saveLeadsLocal(); // Persist fetched leads locally to prevent 'everything zero' on cold refreshes!
    } catch (e) {
...
async function saveLeads(singleLead = null) {
  // Always keep local storage updated in all modes
  saveLeadsLocal();
  ...
  if (supabaseClient) {
    ...
    try {
      const leadsToSave = singleLead ? [singleLead] : leads;
...
function saveCliente() {
  ...
  if (id) {
    ...
    leads.unshift(activeLead);
    toast(`"${nome}" cadastrado com sucesso!`, 'success');
  }

  saveLeads(activeLead);
...
function salvarInteracao() {
  ...
  saveLeads(l); // Single-lead save to avoid bulk sync loops
  $('hist-obs').value = '';
  renderHistoricoList(l);
...
function enviarWaMessage() {
  ...
  saveLeads(lead); // Single-lead save to avoid bulk sync loops
...
/* ============================================================
   ...
   ============================================================ */
async function init() {
  // 1. Sync load local data first so page loads instantly
  loadLeadsLocal();
  await loadWaTemplates();
  setupEvents();
  renderAll();
  ...
  if (isSupaActive) {
    ...
    // If still not logged in, schedule login modal safely
    if (!currentUser) {
      setTimeout(() => {
        if (!currentUser) openAuthModal('login');
      }, 500);
    } else {
      // If logged in, fetch current cloud leads, setup realtime, and render
      await loadLeads();
      setupRealtime();
      renderAll();
    }
